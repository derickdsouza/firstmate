...
The watcher accepts the stock shim only when its bytes match the expected generated content, then invokes the trusted repository poll script directly instead of executing state-file source.
An explicit `bin/fm-x-watch-register.sh` registration can authenticate an alternate shim and exec target; that command's header owns the binding, and homes without a registration stay on the stock generated shim and byte-static dispatch.
