...
#                            or a still-matching registered alternate (see
#                            bin/fm-x-watch-register.sh) without clobbering it
...
x_mode_setup() {
  ...
  if fmx_poll_shim_registered_valid "$shim" "$shim_home" "$FM_ROOT" \
    || fmx_poll_shim_manifest_valid "$shim" "$shim_home"; then
    :
  else
    shim_body=$(fmx_poll_shim_content "$shim_home" "$FM_ROOT")
    x_mode_write_if_changed "$shim" "$shim_body" 700 || { fmx_arm_failed; return 0; }
    fmx_poll_shim_stock_valid "$shim" "$shim_home" "$FM_ROOT" \
      || { fmx_arm_failed; return 0; }
  fi
